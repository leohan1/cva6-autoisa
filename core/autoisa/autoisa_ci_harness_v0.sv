...
module autoisa_ci_harness_v0 (
  ...
  always_comb begin
    ...
    for (int unsigned i = 0; i < AUTOISA_MAX_SRC; i++) begin
      engine_req.operands[i] = (decoded_desc.src_addr[i] == 5'd0) ? 32'd0 : issue_operands_i[i];
    end
  end
  ...
  assign result_valid_o = live_q && engine_done_q && commit_seen_q && !killed_q && !kill_matches;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      end else if (killed_q && (engine_done_q || (engine_rsp_valid && engine_rsp_ready))) begin
        live_q   <= 1'b0;
        killed_o <= 1'b1;
