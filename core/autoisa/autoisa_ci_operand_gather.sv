...
module autoisa_ci_operand_gather #(
  ...
  function automatic logic legal_source_mask(input logic [AUTOISA_MAX_SRC-1:0] mask);
    begin
      unique case (mask)
        6'b00_0001, 6'b00_0011, 6'b00_0111,
        6'b00_1111, 6'b01_1111, 6'b11_1111: legal_source_mask = 1'b1;
  ...
  always_comb begin
    if (active_desc_q.src_valid[4] || active_desc_q.src_valid[5])
      last_beat = 2'd2;
    else if (active_desc_q.src_valid[2] || active_desc_q.src_valid[3])
      last_beat = 2'd1;
    else
      last_beat = 2'd0;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (pending_kill_found)
        pending_valid_q[pending_kill_index] <= 1'b0;
      ...
      if (killed_this_cycle != 0)
        killed_count_o <= killed_count_o + killed_this_cycle;
