  ...
  generate
    if (!MULTI_ENGINE) begin : gen_single_engine
      autoisa_ci_engine_skid #(
          ...
          .in_valid_i(skid_in_valid), .in_ready_o(skid_in_ready),
          .in_req_i(table_dispatch_req),
          .out_valid_o(engine_req_valid), .out_ready_i(engine_req_ready),
          .out_req_o(engine_req),
          .kill_valid_i(kill_valid_i), .kill_tag_i(kill_tag_i),
          .kill_epoch_i(kill_epoch_i), .kill_hit_o(skid_kill_hit),
      ...
      autoisa_ci_dummy_engine i_dummy_engine (
          ...
          .req_valid_i(engine_req_valid), .req_ready_o(engine_req_ready),
          .req_i(engine_req), .rsp_valid_o(engine_rsp_valid),
          .rsp_ready_i(engine_rsp_ready), .rsp_o(engine_rsp)
      );
      ...
      autoisa_ci_multi_engine_cluster #(
          ...
          .clk_i(clk_i), .rst_ni(rst_ni), .flush_i(flush_i),
          .req_valid_i(skid_in_valid), .req_ready_o(skid_in_ready),
          .req_i(table_dispatch_req), .req_supported_o(),
          .kill_valid_i(kill_valid_i), .kill_tag_i(kill_tag_i),
          .kill_epoch_i(kill_epoch_i),
      ...
      assign engine_started_count_o = multi_engine0_dispatch_count +
                                      multi_engine1_dispatch_count;
