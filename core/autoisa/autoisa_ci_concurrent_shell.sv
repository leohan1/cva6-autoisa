...
module autoisa_ci_concurrent_shell #(
  ...
  logic [AUTOISA_TAG_WIDTH-1:0] engine_active_tag_q;
  logic [AUTOISA_EPOCH_WIDTH-1:0] engine_active_epoch_q;
  logic kill_wait_completion;
  logic [$clog2(INFLIGHT_DEPTH+1)-1:0] multi_pending_occupancy;
  logic [$clog2(INFLIGHT_DEPTH+1)-1:0] multi_pending_hwm;
  logic multi_engine0_busy, multi_engine1_busy;
  logic multi_engine0_running, multi_engine1_running;
  logic multi_kill_pending_hit, multi_kill_running_hit;
  logic [COUNT_WIDTH-1:0] multi_accepted_count;
  logic [COUNT_WIDTH-1:0] multi_engine0_dispatch_count;
  logic [COUNT_WIDTH-1:0] multi_engine1_dispatch_count;
  logic [COUNT_WIDTH-1:0] multi_completion_count;
  logic [COUNT_WIDTH-1:0] multi_unsupported_stall_count;
  ...
  assign req_duplicate_o = req_valid_i && (queue_identity_match || table_alloc_duplicate);
  assign req_ready_o = queue_in_ready && table_alloc_ready && !queue_identity_match;
  assign queue_in_valid = req_valid_i && table_alloc_ready && !queue_identity_match;
  assign table_alloc_valid = req_valid_i && queue_in_ready && !queue_identity_match;
  ...
  autoisa_ci_inflight_table #(
      ...
      .kill_wait_completion_i(kill_wait_completion),
  ...
  assign table_dispatch_ready = dispatch_identity_match && skid_in_ready && credit_available;
  ...
  assign tombstone_drop = queue_has_stored_head && queue_out_valid && !dispatch_identity_match;
  assign queue_out_ready = queue_has_stored_head && (tombstone_drop || dispatch_fire);

  generate
    if (!MULTI_ENGINE) begin : gen_single_engine
      ...
      assign kill_wait_completion = kill_valid_i && engine_active_q &&
                                    (engine_active_tag_q == kill_tag_i) &&
                                    (engine_active_epoch_q == kill_epoch_i);

      always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
          engine_active_q <= 1'b0;
          engine_active_tag_q <= '0;
          engine_active_epoch_q <= '0;
        end else begin
          if (engine_req_valid && engine_req_ready) begin
            engine_active_q <= 1'b1;
            engine_active_tag_q <= engine_req.tag;
            engine_active_epoch_q <= engine_req.epoch;
          end
          if (engine_rsp_valid && engine_rsp_ready) engine_active_q <= 1'b0;
        end
      end
    end else begin : gen_multi_engine
      autoisa_ci_multi_engine_cluster #(
          .PENDING_DEPTH(INFLIGHT_DEPTH),
          .COUNT_WIDTH  (COUNT_WIDTH)
      ) i_multi_engine_cluster (
          .clk_i(clk_i),
          .rst_ni(rst_ni),
          .flush_i(flush_i),
          .req_valid_i(skid_in_valid),
          .req_ready_o(skid_in_ready),
          .req_i(table_dispatch_req),
          .req_supported_o(),
          .kill_valid_i(kill_valid_i),
          .kill_tag_i(kill_tag_i),
          .kill_epoch_i(kill_epoch_i),
          .kill_pending_hit_o(multi_kill_pending_hit),
          .kill_running_hit_o(multi_kill_running_hit),
          .rsp_valid_o(engine_rsp_valid),
          .rsp_ready_i(engine_rsp_ready),
          .rsp_o(engine_rsp),
          .pending_occupancy_o(multi_pending_occupancy),
          .pending_high_watermark_o(multi_pending_hwm),
          .engine0_busy_o(multi_engine0_busy),
          .engine1_busy_o(multi_engine1_busy),
          .engine0_running_o(multi_engine0_running),
          .engine1_running_o(multi_engine1_running),
          .accepted_count_o(multi_accepted_count),
          .engine0_dispatch_count_o(multi_engine0_dispatch_count),
          .engine1_dispatch_count_o(multi_engine1_dispatch_count),
          .completion_count_o(multi_completion_count),
          .unsupported_stall_count_o(multi_unsupported_stall_count),
          .pending_kill_drop_count_o(skid_killed_drop_count_o),
          .pending_flush_drop_count_o(skid_flush_drop_count_o)
      );

      assign engine_skid_occupancy_o = (multi_pending_occupancy != '0);
      assign engine_skid_high_watermark_o = (multi_pending_hwm != '0);
      assign skid_kill_hit = multi_kill_pending_hit;
      assign skid_accepted_count = multi_accepted_count;
      assign engine_started_count_o = multi_engine0_dispatch_count + multi_engine1_dispatch_count;
      assign kill_wait_completion = multi_kill_running_hit;
      assign engine_req_valid = 1'b0;
      assign engine_req_ready = 1'b0;
      assign engine_req = '0;
    end
  endgenerate
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (engine_rsp_valid && engine_rsp_ready) completion_count_o <= completion_count_o + 1'b1;
...
`ifndef SYNTHESIS
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      assert (!(kill_credit_release && (reserved_credits_q == '0) && !dispatch_fire))
      else $error("concurrent shell result credit underflow on kill");
      assert (!(result_enqueue && (reserved_credits_q == '0) && !dispatch_fire))
