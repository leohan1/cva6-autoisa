...
module tb_autoisa_ci_cva6_host_transport;
  ...
  task automatic expect_scalar(input logic [TID_W-1:0] id, input logic [4:0] addr,
  ...
  initial begin
    ...
    if (busy_mask[5] || commit_pending[1]) $fatal(1, "scalar terminal event did not release state");
    ...
    if (!busy_mask[8] || !busy_mask[9]) $fatal(1, "pair destination released before first beat");
    repeat (2) begin
      @(posedge clk);
      @(negedge clk);
    ...
    @(posedge clk);
    @(negedge clk);
    if (!wb_valid || wb_id != 2 || wb_addr != 9 || wb_data != 32'hffff_ffff || !wb_we || !wb_last)
      $fatal(1, "pair second WB beat mismatch");
    if (!busy_mask[8] || !busy_mask[9]) $fatal(1, "pair destination released between WB beats");
    @(posedge clk);
    @(negedge clk);
    if (busy_mask[8] || busy_mask[9]) $fatal(1, "pair destination not released after final beat");
    ...
    if (busy_mask[16] || commit_pending[4]) $fatal(1, "kill did not release Host transport state");
    ...
    if (shell_accepted != 3 || shell_retired != 3 || wb_results != 3 || wb_beats != 4)
      $fatal(
          1,
          "transport counters mismatch accepted=%0d retired=%0d results=%0d beats=%0d",
          shell_accepted,
          shell_retired,
          wb_results,
          wb_beats
