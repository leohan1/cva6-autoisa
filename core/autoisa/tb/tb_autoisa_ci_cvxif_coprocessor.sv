...
module tb_autoisa_ci_cvxif_coprocessor;
  ...
  task automatic issue_commit(input id_t id, input logic [31:0] instr, input logic [31:0] a,
                              input logic [31:0] b);
    begin
      @(negedge clk);
      req.issue_valid = 1;
      req.register_valid = 1;
      req.commit_valid = 1;
      req.issue_req.instr = instr;
      req.issue_req.id = id;
      req.issue_req.hartid = 32'h1234;
      req.register.id = id;
      req.register.hartid = 32'h1234;
      req.register.rs[0] = a;
      req.register.rs[1] = b;
      req.register.rs_valid = 2'b11;
      req.commit.id = id;
      req.commit.hartid = 32'h1234;
      ...
      req.issue_valid = 0;
      req.register_valid = 0;
      #1;
      if (dut.commit_valid) $fatal(1, "held CV-X-IF commit was forwarded twice id=%0d", id);
      @(posedge clk);
      @(negedge clk);
      req.commit_valid = 0;
    end
  ...
  initial begin
    ...
    req.issue_valid = 1;
    req.register_valid = 1;
    req.issue_req.instr = enc_l3(3, 8, 1, 2);
    req.issue_req.id = 2;
    req.register.rs_valid = 2'b11;
    #1;
    if (!resp.issue_ready || resp.issue_resp.accept)
