...
module autoisa_ci_axi_memory #(
  ...
  function automatic ariane_axi::addr_t beat_addr(
    ...
    begin
      ...
      if (burst == axi_pkg::BURST_WRAP) begin
        boundary = (base >> (size + $clog2(len + 1))) <<
                   (size + $clog2(len + 1));
  ...
  wire ariane_axi::addr_t rd_addr =
      beat_addr(rd_base_q, rd_size_q, rd_len_q, rd_burst_q, rd_beat_q);
  wire ariane_axi::addr_t wr_addr =
      beat_addr(wr_base_q, wr_size_q, wr_len_q, wr_burst_q, wr_beat_q);

  always_comb begin
    merged_write = tohost_shadow_q;
    for (int unsigned byte_idx = 0; byte_idx < DATA_BYTES; byte_idx++)
      if (req_i.w.strb[byte_idx])
        merged_write[byte_idx*8 +: 8] = req_i.w.data[byte_idx*8 +: 8];
  end
...
module tb_autoisa_ci_ariane_elf;
  ...
  ariane #(
    ...
    .clk_i(clk_i), .rst_ni(rst_ni), .boot_addr_i(BOOT_ADDR),
    .hart_id_i('0), .irq_i('0), .ipi_i(1'b0), .time_irq_i(1'b0),
    .debug_req_i(1'b0), .rvfi_probes_o(),
    .noc_req_o(noc_req), .noc_resp_i(noc_resp)
  );

  autoisa_ci_axi_memory i_memory (
    .clk_i(clk_i), .rst_ni(rst_ni), .req_i(noc_req), .resp_o(noc_resp),
    .tohost_valid_o(tohost_valid), .tohost_value_o(tohost_value)
  );
