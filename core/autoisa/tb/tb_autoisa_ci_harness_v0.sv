...
module tb_autoisa_ci_harness_v0;
  ...
  function automatic logic [31:0] enc_l7(input logic [9:0] imm10);
    // Layout v2 scattered immediate: imm[9:3] -> [31:25],
    // imm[2:0] -> [14:12], custom-0 separates it from L0-L4.
    enc_l7 = ({22'd0, imm10[9:3]} << 25) | (32'd2 << 20) |
             (32'd1 << 15) | ({29'd0, imm10[2:0]} << 12) |
             (32'd5 << 7) | 32'h0b;
  endfunction
  ...
  initial begin
    ...
    run_ci(enc_l7(10'd2), 4'd8, 2'b01, 32'd5, 32'd0);
