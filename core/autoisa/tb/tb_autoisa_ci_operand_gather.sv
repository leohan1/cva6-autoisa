...
module tb_autoisa_ci_operand_gather;
  ...
  autoisa_ci_operand_gather #(.PENDING_DEPTH(4)) dut (
      .clk_i(clk), .rst_ni(rst_n), .flush_i(flush),
      .desc_valid_i(desc_valid), .desc_ready_o(desc_ready), .desc_i(desc),
      .desc_illegal_o(desc_illegal), .rf_req_valid_o(rf_req_valid),
      .rf_req_ready_i(rf_req_ready), .rf_lane_valid_o(rf_lane_valid),
      .rf_addr_o(rf_addr), .rf_data_i(rf_data), .req_valid_o(req_valid),
      .req_ready_i(req_ready), .req_o(req), .kill_valid_i(kill_valid),
      .kill_tag_i(kill_tag), .kill_epoch_i(kill_epoch), .kill_hit_o(kill_hit),
      .occupancy_o(occupancy), .high_watermark_o(hwm),
      .accepted_count_o(accepted), .emitted_count_o(emitted),
      .gather_beat_count_o(beats), .killed_count_o(killed),
      .flush_drop_count_o(flush_drop)
  );

  task automatic make_desc(input logic [3:0] tag, input int unsigned nsrc,
  ...
  task automatic submit(input logic [3:0] tag, input int unsigned nsrc,
                        input logic [4:0] base);
    begin
      @(negedge clk); make_desc(tag, nsrc, base); desc_valid = 1;
      #1;
      while (!desc_ready) begin @(negedge clk); #1; end
      if (desc_illegal) $fatal(1, "legal descriptor rejected tag=%0d", tag);
      @(posedge clk); @(negedge clk); desc_valid = 0;
    end
  endtask

  task automatic consume(input logic [3:0] tag, input int unsigned nsrc,
                         input logic [4:0] base);
    ...
    begin
      ...
      req_ready = 1;
      @(posedge clk); @(negedge clk); req_ready = 0;
    end
  endtask

  task automatic do_kill(input logic [3:0] tag);
    begin
      @(negedge clk); kill_tag = tag; kill_epoch = 0; kill_valid = 1; #1;
      if (!kill_hit) $fatal(1, "kill missed gather tag=%0d", tag);
      @(posedge clk); @(negedge clk); kill_valid = 0;
    end
  endtask

  initial begin
    flush = 0; desc_valid = 0; desc = '0; rf_req_ready = 1;
    req_ready = 0; kill_valid = 0; kill_tag = 0; kill_epoch = 0;
    ...
    submit(13, 6, 2);
    wait (rf_req_valid);
    @(negedge clk); rf_req_ready = 1;
    @(posedge clk); @(negedge clk); rf_req_ready = 0;
    do_kill(13);
    ...
    if ((accepted != 15) || (emitted != 11) || (beats != 20) ||
      ...
      $fatal(1, "gather counters mismatch accepted=%0d emitted=%0d beats=%0d killed=%0d flush=%0d hwm=%0d",
