...
module tb_autoisa_ci_multi_engine_shell;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (inflight_hwm > observed_inflight_hwm) observed_inflight_hwm <= inflight_hwm;
      if (credit_hwm > observed_credit_hwm) observed_credit_hwm <= credit_hwm;
    end
  end
  ...
  autoisa_ci_concurrent_shell #(
      ...
      .request_high_watermark_o(request_hwm),
      .inflight_occupancy_o(inflight_occupancy),
      .inflight_high_watermark_o(inflight_hwm),
      .result_occupancy_o(result_occupancy),
      .result_high_watermark_o(result_hwm),
      .reserved_result_credits_o(reserved_credits),
      .credit_high_watermark_o(credit_hwm),
      .engine_skid_occupancy_o(pending_occupancy),
      .engine_skid_high_watermark_o(pending_hwm),
      .accepted_count_o(accepted),
      .dispatched_count_o(dispatched),
      .engine_started_count_o(started),
      .completion_count_o(completed),
      .retired_count_o(retired),
      .killed_count_o(killed),
      .orphan_completion_count_o(orphan),
      .tombstone_drop_count_o(tombstone_drop),
      .credit_stall_count_o(credit_stall),
      .result_full_stall_count_o(result_full_stall),
  ...
  task automatic submit(input logic [3:0] tag, input logic [7:0] ci, input logic [31:0] a,
                        input logic [31:0] b);
    begin
      @(negedge clk);
      req = '0;
  ...
  task automatic consume(input logic [3:0] tag, input logic [31:0] value);
    begin
      ...
      if ((result.tag != tag) || (result.status != AUTOISA_STATUS_OK) ||
        ...
        $fatal(
            1,
            "result mismatch expected tag=%0d value=%h got tag=%0d value=%h",
            tag,
            value,
            result.tag,
            result.results[0]
  ...
  initial begin
    ...
    if ((accepted != 8) || (dispatched != 8) || (started != 6) ||
      ...
      $fatal(
          1,
          "multi-shell accounting acc=%0d disp=%0d start=%0d comp=%0d ret=%0d kill=%0d orphan=%0d pkill=%0d pflush=%0d credit=%0d",
          accepted,
          dispatched,
          started,
          completed,
    ...
    $display(
        "DATA: accepted=%0d dispatched=%0d started=%0d completed=%0d retired=%0d killed=%0d orphan=%0d pending_kill=%0d pending_flush=%0d inflight_hwm=%0d credit_hwm=%0d",
        accepted, dispatched, started, completed, retired, killed, orphan, pending_kill_drop,
        pending_flush_drop, observed_inflight_hwm, observed_credit_hwm);
  ...
  initial begin
    ...
    $fatal(
        1,
        "timeout acc=%0d disp=%0d start=%0d comp=%0d ret=%0d kill=%0d orphan=%0d rq=%0d inf=%0d res=%0d pending=%0b credits=%0d result_valid=%0b result_tag=%0d reqv=%0b reqtag=%0d ready=%0b qready=%0b tready=%0b qmatch=%0b tdup=%0b",
        accepted, dispatched, started, completed, retired, killed, orphan, request_occupancy,
        inflight_occupancy, result_occupancy, pending_occupancy, reserved_credits, result_valid,
        result.tag, req_valid, req.tag, req_ready, dut.queue_in_ready, dut.table_alloc_ready,
        dut.queue_identity_match, dut.table_alloc_duplicate);
  end
endmodule
