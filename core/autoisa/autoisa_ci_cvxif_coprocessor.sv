...
module autoisa_ci_cvxif_coprocessor #(
  ...
  autoisa_ci_engine_descriptor i_descriptor (
      .ci_id_i(decoded_desc.ci_id),
      .supported_o(engine_supported),
  ...
  always_comb begin
    ...
    for (int unsigned i = 0; i < AUTOISA_MAX_SRC; i++) begin
      if ((i >= NR_RS) && decoded_desc.src_valid[i]) source_shape_supported = 1'b0;
      if ((i < NR_RS) && decoded_desc.src_valid[i] && !cvxif_req_i.register.rs_valid[i])
        operands_valid = 1'b0;
    end
  end
  ...
  always_comb begin
    ...
    cvxif_resp_o.issue_resp.accept = native_eligible && operands_valid && !live_q[issue_id];
    ...
    cvxif_resp_o.issue_resp.register_read[i] = native_eligible && decoded_desc.src_valid[i];
  ...
  always_comb begin
    ...
    for (int unsigned i = 0; i < NR_RS; i++)
    if (i < AUTOISA_MAX_SRC) shell_req.operands[i] = cvxif_req_i.register.rs[i][31:0];
  end
  ...
  assign kill_valid = cvxif_req_i.commit_valid && cvxif_req_i.commit.commit_kill && commit_live;
  ...
  autoisa_ci_concurrent_shell #(
      ...
      .kill_tag_i(commit_tag),
      .kill_epoch_i(commit_epoch),
      .kill_hit_o(),
      .result_valid_o(shell_result_valid),
      .result_ready_i(cvxif_req_i.result_ready),
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      epoch_q <= '0;
      rd_q <= '0;
      for (int unsigned i = 0; i < TRANS_IDS; i++) hartid_q[i] <= '0;
    end else begin
      ...
      if (commit_valid) committed_q[commit_id] <= 1'b1;
  ...
  initial begin
    assert (XLEN == 32)
    else $error("AutoISA CV-X-IF bridge currently requires XLEN=32");
